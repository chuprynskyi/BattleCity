...
namespace RenderEngine
{
	...
	void VertexArray::addBuffer(const VertexBuffer& vertexBuffer, const VertexBufferLayout& layout)
	{
		...
		for (int i = 0; i < layoutElements.size(); ++i)
		{
			...
			glVertexAttribPointer(currentAtribIndex, currentLayoutElement.count, currentLayoutElement.type, currentLayoutElement.normalized, static_cast<unsigned int>(layout.getStride()), offset);
