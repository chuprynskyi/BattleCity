                                                                              ...
                                                                              const std::string& subTextureName)
{
    ...
    if (!pShader)
    {
        std::cerr << "Can't find the shader program: " << shaderName << " for animated sprite: " << spriteName << std::endl;
    }

    std::shared_ptr<Renderer::AnimatedSprite> newAnimatedSprite = m_animatedSprites.emplace(textureName, std::make_shared<Renderer::AnimatedSprite>(pTexture,
        subTextureName,
        pShader,
        glm::vec2(0.f, 0.f),
        glm::vec2(spriteWidth, spriteHeight))).first->second;
